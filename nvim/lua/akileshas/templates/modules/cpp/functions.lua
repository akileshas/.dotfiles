...
function M.toggle_input_output_files()
	...
	for _, win_id in ipairs(api.nvim_list_wins()) do
		...
		if buf_name == "/home/akileshas/personal/AlgoX/input.txt" then
			is_input_open = true
			input_win_id = win_id
		elseif buf_name == "/home/akileshas/personal/AlgoX/output.txt" then
			is_output_open = true
			output_win_id = win_id
		elseif buf_name == "/home/akileshas/personal/AlgoX/compiler_error.txt" then
			is_error_open = true
			error_win_id = win_id
		end
	end
